...
generate.B.mat <- function(n, p, mu, sigma, corr = 0){
  ...
  # corr      <- correlation/rho in the covariance matrix of B
  #----------------------------------------------------------------------------#
  # OUTPUT: 
  ...
  Sigma.Mat <- diag(1, p) + corr *
    (outer(rep(1, p), rep(1, p)) - diag(1, p))
  
  for (j in 1:n){
    
    B[ ,j] <- (sqrt(mu/n)*sigma[j]) * rnorm(p, mean = 0, sd = 1) + 
      MASS::mvrnorm(1, mu = rep(0, p), Sigma = Sigma.Mat)
...
power.comp <- function(n, m, p, corr, gamma, sigma, mu, lambda.vec, d.vec,
                       n.iter = 50){
  ...
  B.mat <- generate.B.mat(n, p, mu, sigma, corr)
  ...
  (overlap <- abs(sum(sigma.hat * sigma))/n)
